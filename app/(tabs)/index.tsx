import { auth, db } from '@/app/utils/firebaseConfig'; // make sure this points to your config
...
const StreakScreen = () => {
  ...
  const [streakPhotos, setStreakPhotos] = useState({});
  
  // New state for hard mode functionality
  const [isHardMode, setIsHardMode] = useState(false);
  const [showSettings, setShowSettings] = useState(false);
  const [morningCheckIn, setMorningCheckIn] = useState({ completed: false, photo: null });
  const [eveningCheckIn, setEveningCheckIn] = useState({ completed: false, photo: null });
  const [currentCheckInType, setCurrentCheckInType] = useState('morning'); // 'morning' or 'evening'
  
  // Calendar state
  const [showCalendar, setShowCalendar] = useState(false);
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [checkedInDates, setCheckedInDates] = useState(new Set());
  ...
  const loadCheckedInDates = async () => {
    try {
      ...
      if (savedDates) {
        setCheckedInDates(new Set(JSON.parse(savedDates)));
      }
    } catch (error) {
  ...
  const saveCheckedInDate = async (date) => {
    try {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (hardMode !== null) {
        setIsHardMode(JSON.parse(hardMode));
      }
    } catch (error) {
  ...
  const saveSettings = async (hardMode) => {
    try {
  ...
  const loadDailyCheckIns = async () => {
    try {
      ...
      if (morningData) {
        setMorningCheckIn(JSON.parse(morningData));
      } else {
        setMorningCheckIn({ completed: false, photo: null });
      }
      
      if (eveningData) {
        setEveningCheckIn(JSON.parse(eveningData));
      } else {
  ...
  const saveDailyCheckIn = async (type, data) => {
    try {
      const today = format(new Date(), 'yyyy-MM-dd');
  ...
  const loadStreakPhotos = async () => {
    try {
      const savedPhotos = await AsyncStorage.getItem('streakPhotos');
      if (savedPhotos) {
        setStreakPhotos(JSON.parse(savedPhotos));
      }
    } catch (error) {
  ...
  const savePhotoToStorage = async (photoUri, type = 'single') => {
    try {
      const today = format(new Date(), 'yyyy-MM-dd');
  ...
  const loadStreak = async () => {
    try {
      ...
      const hardMode = JSON.parse((await AsyncStorage.getItem("isHardMode")) || "false");
      ...
      if (lastOpened) {
        ...
        if (dayDifference === 0) {
          if (hardMode) {
            ...
            const morning = morningData ? JSON.parse(morningData) : { completed: false };
            const evening = eveningData ? JSON.parse(eveningData) : { completed: false };
  ...
  const uploadPhoto = async (type = 'single') => {
  ...
  const handleEasyModeCheckIn = async () => {
    ...
    try {
      ...
      await saveStreakToFirestore(streak, morningCheckIn, eveningCheckIn, streakPhotos);
  ...
  const handleHardModeCheckIn = async () => {
    ...
    try {
      ...
      const checkInData = { completed: true, photo: selectedPhoto.uri };
  ...
  const getTodayPhotoUri = (type = 'single') => {
    const today = format(new Date(), 'yyyy-MM-dd');
    const photoKey = type === 'single' ? today : `${today}_${type}`;
  ...
  const canCheckIn = () => {
    if (!isHardMode) {
      return !hasCheckedInToday && selectedPhoto;
    } else {
      if (currentCheckInType === 'morning') {
        return !morningCheckIn.completed && selectedPhoto;
      } else {
        return !eveningCheckIn.completed && selectedPhoto;
      }
    }
  };
  ...
  const saveStreakToFirestore = async (streakCount, morningCheckIn, eveningCheckIn, streakPhotos) => {
    ...
    try {
      await setDoc(userRef, {
        ...
        lastCheckin: serverTimestamp(), // <— this updates with Firestore timestamp
        dailyCheckIns: {
